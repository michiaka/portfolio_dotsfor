...
bool addReview(char *inputstr, OuterArr *dates, DATE *storedDate, int *i, int *j)
{
    ...
    if((ret = sscanf(inputstr," + %d-%02d-%02d %ld %4096s", &review.curDate.year, &review.curDate.month,
    ...
    char extra = 0;
    if(sscanf(inputstr, "+ %*d-%*d-%*d %*d %*s %c", &extra) == 1 && extra != '\0') 
    {
        // printf("Invalid input.\n");
        printf("Invalid input.\n");
...
int main()
{
    ...
    while(getline(&buffer, &capa, stdin) != -1)
    {
